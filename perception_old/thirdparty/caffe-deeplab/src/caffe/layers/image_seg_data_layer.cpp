...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  const int channels2 = cv_img2.channels();
  const int height2 = cv_img2.rows;
  const int width2 = cv_img2.cols;
  ...
  if (crop_width > 0 && crop_height > 0) {
    ...
    //label
    top[1]->Reshape(batch_size, 1, height2, width2);
    this->transformed_label_.Reshape(batch_size, 1, height2, width2);
    for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
      this->prefetch_[i].label_.Reshape(batch_size, 1, height2, width2);
    }
  }
